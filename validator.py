...
class Validator:       # classe do validador da gramática
    ...
    def _validate_symbol(self, symbol, node):       # valida simbolo da gramática com recurssão, produz a árvore da análise
            ...
            for char in production:
                if char in self.non_terminals:
                    ...
                    if self.assert_char(char):
                        ...
                        matched = False
                        error_message = f"Erro ao validar símbolo após posição {self.position}"
                        error_position = self.position
                        break
        ...
        return False, error_message or f"Erro ao validar símbolo após posição {self.position}", node, error_position

def print_tree(node, level=0): #imprime a árvore gerada pelo analisador
    if node is not None:
